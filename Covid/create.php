...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($input_id)) {
        ...
        if($input_id != "select id from people "){
            $id = $input_id;
        }else{
    ...
    if(empty($id_err)&&empty($name_err)&&empty($tel_err)&&empty($address_err)&&empty($F_err)&&empty($location_err)&&empty($start_date_err)&&empty($end_date_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            mysqli_stmt_bind_param($stmt,"ssssssss",$param_id, $param_name, $param_tel, $param_address, $param_F, $param_location, $param_start_date, $param_end_date);
